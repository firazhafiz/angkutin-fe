...
export default function StatsCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute -right-6 -top-6 h-28 w-28 rounded-full bg-gradient-to-br from-soft-gray to-transparent opacity-40 transition-transform duration-500 group-hover:scale-150" />
        ...
        <div className="flex flex-col gap-1.5 mt-1">
          ...
          {(subtitle || trend) && (
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {subtitle && (
                <span className="text-[11px] font-medium text-gray-400">{subtitle}</span>
              )}
              {trend && !subtitle && (
                <span className="text-[11px] font-medium text-gray-400">vs minggu lalu</span>
              )}
            </div>
          )}
        </div>
