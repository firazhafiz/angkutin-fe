...
import { Search, Eye, Ban, Plus, Trash2, Pencil, RefreshCcw, Truck, CheckCircle2 } from 'lucide-react';
...
export default function CouriersPage() {
  ...
  const couriers = rawData.map((c: any) => {
    const isActuallySuspended = (c.user?.status || c.status || '').toUpperCase() === 'SUSPENDED' || c.isSuspended === true;
    ...
    console.log(`[CouriersPage] Mapping ${mapped.name}: isSuspended=${mapped.isSuspended} (raw status=${c.user?.status || c.status}, raw isSuspended=${c.isSuspended})`);
    return mapped;
  }) as ExtendedCourier[];
  ...
  const courierDetail = detailResponse?.data ? {
    ...
    userId: (detailResponse.data as any).user?.id || (detailResponse.data as any).userId || (detailResponse.data as any).id || '',
    name: (detailResponse.data as any).user?.name || (detailResponse.data as any).name || 'Unknown',
    email: (detailResponse.data as any).user?.email || (detailResponse.data as any).email || '',
    phone: (detailResponse.data as any).user?.phone || (detailResponse.data as any).phone || '',
    isSuspended: ((detailResponse.data as any).user?.status || (detailResponse.data as any).status || '').toUpperCase() === 'SUSPENDED' || (detailResponse.data as any).isSuspended || false,
  } : null;
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      const msg = error.response?.data?.message || error.response?.data?.error || 'Gagal menambahkan kurir';
      toast.error(msg);
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => adminService.updateCourier(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['couriers'] });
      setIsFormModalOpen(false);
    ...
    onError: (error: any) => {
      const msg = error.response?.data?.message || error.response?.data?.error || 'Gagal memperbarui kurir';
      toast.error(msg);
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => toast.error(error.response?.data?.message || 'Gagal menghapus kurir'),
  });

  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status, name, phone }: { id: string; status: 'ACTIVE' | 'SUSPENDED', name?: string, phone?: string }) => {
      console.log('--- Mutation Triggered ---');
      console.log('Target ID:', id);
      console.log('Payload:', { name, phone, status });
      return adminService.updateUser(id, { name, phone, status });
    },
    onSuccess: (data) => {
      console.log('Mutation Success:', data);
      queryClient.invalidateQueries({ queryKey: ['couriers'] });
      queryClient.invalidateQueries({ queryKey: ['courier-detail'] });
      toast.success('Status kurir berhasil diperbarui');
    },
    onError: (error: any) => {
      console.error('Mutation Error:', error);
      const msg = error.response?.data?.message || error.response?.data?.error || 'Gagal memperbarui status kurir';
      toast.error(msg);
    },
  });
  ...
  const toggleSuspend = (courier: ExtendedCourier) => {
    console.log('--- toggleSuspend Triggered ---');
    console.log('Courier:', courier.name);
    console.log('Current Status (isSuspended):', courier.isSuspended);

    const isActuallySuspended = courier.isSuspended;
    const newStatus = isActuallySuspended ? 'ACTIVE' : 'SUSPENDED';
    const actionText = isActuallySuspended ? 'mengaktifkan kembali' : 'menangguhkan (suspend)';
    
    // Use user ID if available, otherwise fallback to id
    const targetId = courier.userId || courier.id;

    console.log('Calculated New Status:', newStatus);
    console.log('Target ID:', targetId);

    if (confirm(`Apakah Anda yakin ingin ${actionText} kurir ${courier.name}?`)) {
      console.log('Attempting to toggle suspend for:', courier.name);
  ...
  const ActionButtons = ({ 
    ...
    onToggleStatus: (c: ExtendedCourier) => void,
    onDelete: (id: string) => void
  }) => {
    console.log('ActionButtons Rendered for:', courier.name);
        ...
        <div className="flex items-center justify-center gap-1">
          ...
          <button
            onClick={(e) => {
              ...
              onToggleStatus(courier);
            }}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-emerald-500 hover:bg-emerald-50 hover:text-emerald-700 transition-colors"
            title="Aktifkan Akun"
          >
  ...
  const courierColumns: Column<ExtendedCourier>[] = [
    ...
    {
      key: 'isSuspended', header: 'Status Akun', align: 'center',
      render: (item) => (
        <span className={cn(
          "px-2.5 py-0.5 rounded-full text-xs font-medium",
          !item.isSuspended ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"
        )}>
          {!item.isSuspended ? 'Aktif' : 'Suspended'}
        </span>
      )
    },
    {
      key: 'isOnline', header: 'Status Kerja', align: 'center',
      render: (item) => (
        <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold',
          item.isOnline ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-50 text-gray-400'
        )}>
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-soft-gray bg-white shadow-sm overflow-hidden min-h-[300px] relative">
        ...
        <DataTable
          ...
          keyExtractor={(i) => i.id}
          emptyMessage={isLoading ? "Memuat data..." : "Tidak ada kurir ditemukan"}
        />
      </div>
      ...
      >
        <form onSubmit={handleFormSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input name="name" label="Nama Lengkap" defaultValue={selectedCourier?.name} placeholder="Nama Kurir" required={!isEditMode} disabled={isEditMode} />
            <Input name="phone" label="No. Telepon" defaultValue={selectedCourier?.phone} placeholder="Nomor HP Kurir" required={!isEditMode} disabled={isEditMode} />
          </div>
          <Input name="email" type="email" label="Alamat Email" defaultValue={selectedCourier?.email} placeholder="Email Kurir" required={!isEditMode} disabled={isEditMode} />
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-5 border-b border-soft-gray pb-5">
              <div className={cn("flex h-20 w-20 items-center justify-center rounded-full text-3xl font-bold text-white shadow-md",
                courierDetail.isOnline ? "bg-gradient-to-br from-primary to-secondary" : "bg-gray-400")}>
                {courierDetail.name.charAt(0)}
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-bold text-dark">{courierDetail.name}</h3>
                  <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold',
                    courierDetail.isOnline ? 'bg-primary-light text-primary' : 'bg-soft-gray text-gray-500'
                  )}>
                    <span className={cn('h-1.5 w-1.5 rounded-full', courierDetail.isOnline ? 'bg-primary' : 'bg-gray-400')} />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="rounded-xl bg-primary-light p-4 border border-primary/20">
                ...
                <p className="mt-1 font-bold text-dark capitalize">{(courierDetail.vehicleType || '').replace('_', ' ')}</p>
              </div>
              <div className="rounded-xl bg-soft-gray p-4 border border-gray-200">
                ...
                <p className="mt-1 font-bold text-dark">{courierDetail.vehiclePlate || '-'}</p>
              </div>
              <div className="rounded-xl bg-very-light-gray p-4 border border-soft-gray">
                ...
                <p className="mt-1 font-bold text-dark">{courierDetail.phone}</p>
              </div>
              <div className="rounded-xl bg-primary-light/50 p-4 border border-primary/10">
                ...
                <p className="mt-1 font-bold text-dark">
                  {new Date(courierDetail.createdAt).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}
                </p>
              </div>
            </div>
            ...
            */}
            ...
            <div className="pt-4 flex justify-end gap-3 border-t border-soft-gray">
              {courierDetail!.isSuspended ? (
                <Button
                  className="bg-emerald-500 hover:bg-[#047857] border-emerald-500 text-white gap-2"
                  onClick={() => toggleSuspend(courierDetail as any)}
                ...
                <Button
                  variant="outline"
                  className="text-red-500 hover:bg-red-50 hover:text-red-600 border-red-200 gap-2"
                  onClick={() => toggleSuspend(courierDetail as any)}
                  disabled={updateStatusMutation.isPending}
                >
                  <Ban size={16} /> Suspend Akun
                </Button>
              )}
