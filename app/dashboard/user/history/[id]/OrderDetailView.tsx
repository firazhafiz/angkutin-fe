...
const STATUS_TIMELINE_MAP: Record<string, string> = {
  ...
  [OrderStatus.DELIVERING]: "Menuju Drop Point",
  [OrderStatus.COMPLETED]: "Pesanan Selesai",
  [OrderStatus.CANCELLED]: "Pesanan Dibatalkan",
};

// Ordered status sequence for timeline rendering
...
function buildTimeline(order: Order) {
  // If we have statusHistory from BE, use that
  ...
  // Fallback: build from current status position in sequence
  const currentIdx = STATUS_SEQUENCE.indexOf(order.status);
  const isCancelled = order.status === OrderStatus.CANCELLED;

  const timeline = STATUS_SEQUENCE.map((s, idx) => ({
  ...
  if (isCancelled) {
    return [
      {
        status: STATUS_TIMELINE_MAP[OrderStatus.CREATED],
                  ...
                  {allItems.length > 0 ? (
                    ...
                    <div className="p-6 border border-dashed border-gray-200 rounded-xl text-center">
                      <p className="text-xs text-gray-400 font-medium">
                        Rincian item akan muncul setelah penimbangan.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="w-full mt-2 p-6 bg-primary/5 rounded-xl border border-primary/10 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-black text-dark tracking-tighter">
                        {formatCurrency(order.netTotal || order.totalCredit)}
                      </p>
                    </div>
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="flex-1">
                  <h4 className="font-black text-dark text-lg leading-tight">
                    {order.courier?.user?.name || "Belum ditentukan"}
                  </h4>
